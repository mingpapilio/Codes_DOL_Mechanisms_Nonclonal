 ...
 ************************************************************************************************
 * Output files:
 * summary: A table of last averaged population properties 
 * calculated from N_reps and in the last length_log generations
 ************************************************************************************************
 ...
 * Essentiality of helping is automatically assigned from essential_list
 */
...
int main(){
    ...
    // General parameters
        ...
        int N_rep= 2;                   // Number of repetitions
        int n= 4;                       // Number of individual in each social group
        int N_alloc= 5*n;               // Number of allocating divisions, 
        int N_ini_idvl= 1;              // Number of initial group members (Relatedness= its inverse)
        ...
        double theta= 0.025;            // Scaling coefficient of cost function
        ...
        int length_log= 0.1*T;          // Recording the data from the last 10% of generations
    ...
    // Parameters for repeats
        int length_param= 10;
        double essential_list[length_param], num_ini_idvl[length_param];
        for(i_1=0; i_1< length_param; ++i_1){
            essential_list[i_1]= 0.5+ i_1*(0.5)/(length_param-1);
            num_ini_idvl[i_1]= i_1+ 1.0;
        }
    ...
    // Temporal space
        ...
        double **ProbIdvl= d_array2d(N_rep,G*n);        // Probability of each individual to be chosen to survive (relative fitness)
        double **PorpDiv1= d_array2d(N_rep, G);         // Proportion of helpers in the population (phenotype)
        double ****link= d_array4d(N_rep, G, n, n);     // Linkage (coordination) matrix of a social group
        //
        double globalQ[N_rep], globalQ_sd[N_rep], globalH1[N_rep], globalH1_sd[N_rep], globalH2[N_rep], globalH2_sd[N_rep], globalS[N_rep], globalS_sd[N_rep], globalDiv1[N_rep], globalDiv1_sd[N_rep];
        //
        double log_avg_q[length_log], log_asd_q[length_log], log_avg_h1[length_log], log_asd_h1[length_log], log_avg_h2[length_log], log_asd_h2[length_log], log_avg_s[length_log], log_asd_s[length_log], log_avg_div1[length_log], log_asd_div1[length_log];
        double log_sd_avg_q[length_log], log_sd_asd_q[length_log], log_sd_avg_h1[length_log], log_sd_asd_h1[length_log], log_sd_avg_h2[length_log], log_sd_asd_h2[length_log], log_sd_avg_div1[length_log], log_sd_asd_div1[length_log];
        //
    ...
    // Output file
        ...
        fprintf(out, "essentiality\tgrp_size\tavg_q\tasd_q\tavg_s\tasd_s\tavg_porp\tasd_porp\n");
    
    // Initialization
    for(i_1=0; i_1< length_param; ++i_1){
        e= essential_list[i_1];
        t=0;
        ...
        for(t=1; t<=T; ++t){
            ...
            // Establish links, allocate divisions of labour, calculate fitness
                ...
                // Fitness calculation
                        ...
                        if(t> (T-length_log-1)) PorpDiv1[rep][i]= (double)k_num[rep]/n;
            ...
            // Calculate population values
                for(rep=0; rep<N_rep; rep++){
                    ...
                    if(t> (T-length_log-1)){
                        ...
                        // Swapping the traits, trait 1 always does more helping
                ...
                if(t> (T-length_log-1)){
                    log_avg_q[t-T+length_log]= Mean_array(globalQ, N_rep);
                    log_asd_q[t-T+length_log]= Mean_array(globalQ_sd, N_rep);
                    log_avg_h1[t-T+length_log]= Mean_array(globalH1, N_rep);
                    log_asd_h1[t-T+length_log]= Mean_array(globalH1_sd, N_rep);
                    log_avg_h2[t-T+length_log]= Mean_array(globalH2, N_rep);
                    log_asd_h2[t-T+length_log]= Mean_array(globalH2_sd, N_rep);
                    log_avg_s[t-T+length_log]= Mean_array(globalS, N_rep);
                    log_asd_s[t-T+length_log]= Mean_array(globalS_sd, N_rep);
                    log_avg_div1[t-T+length_log]= Mean_array(globalDiv1, N_rep);
                    log_asd_div1[t-T+length_log]= Mean_array(globalDiv1_sd, N_rep);
                }
            // Print final results
                if(t== (T-1)){
                    avg_Q= Mean_array(log_avg_q, length_log);
                    asd_Q= Mean_array(log_asd_q, length_log);
                    avg_H1= Mean_array(log_avg_h1, length_log);
                    asd_H1= Mean_array(log_asd_h1, length_log);
                    avg_H2= Mean_array(log_avg_h2, length_log);
                    asd_H2= Mean_array(log_asd_h2, length_log);
                    avg_S= Mean_array(log_avg_s, length_log);
                    asd_S= Mean_array(log_asd_s, length_log);
                    avg_porp= Mean_array(log_avg_div1, length_log);
                    asd_porp= Mean_array(log_asd_div1, length_log);
                    fprintf(out, "%lf\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", e, n, avg_Q, asd_Q, avg_S, asd_S, avg_porp, asd_porp);
                }
    }}
    free_d_array4d(Popn);
